...
export default function App() {
  ...
  const processFile = async (file, type) => {
    setScn(true);
    setSs("Ottimizzazione istantanea...");
    
    try {
      // 1. Convertiamo in bitmap per manipolarlo
      ...
      // 2. Creiamo un canvas piccolo per "alleggerire" il file drasticamente
      ...
      // 3. Esportiamo in JPG a bassa densità (ma leggibile per l'AI)
      const compressedBase64 = canvas.toDataURL("image/jpeg", 0.5).split(",")[1];

      setSs("Analisi colonne...");
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: "text", text: `Analizza questa pagina Kids&Us (impaginazione a DUE COLONNE). 
    ...
    <div style={{ minHeight: "100vh", fontFamily: "'Nunito', sans-serif", background: "#FAFAF7", padding: 20 }}>
      ...
      {view === "course" && sc && (
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          ...
          <h2 style={{ color: sc.color, fontWeight: 900 }}>{sc.name}</h2>
          ...
          <input type="file" ref={fr} style={{ display: "none" }} accept="image/*" onChange={e => processFile(e.target.files[0], "r")} />
        </div>
      )}
      ...
      {view === "lesson" && (
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          ...
          <div style={{ background: "#fff", padding: 25, borderRadius: 24, marginBottom: 20, textAlign: "center", border: "1px solid #eee" }}>
             ...
             <button onClick={() => fr.current.click()} style={{ width: "100%", background: sc.color, color: "#fff", padding: "14px", borderRadius: 14, border: "none", fontWeight: 800 }}>📸 SCANSIONA DAY {sd}</button>
             <input type="file" ref={fr} style={{ display: "none" }} accept="image/*" onChange={e => processFile(e.target.files[0], "l")} />
          </div>
      ...
      {scn && <div style={{ position: "fixed", bottom: 20, left: "50%", transform: "translateX(-50%)", background: "#3C3C3B", color: "#fff", padding: "12px 24px", borderRadius: 50 }}>{ss}</div>}
    </div>
  );
}
